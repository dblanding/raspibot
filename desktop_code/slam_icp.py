...
if __name__ == "__main__":
    ...
    for n in range(0, len(data)-2, 2):
        ...
        prev_scan = (data[n]['scan'])  # in local polar coords
        prev_scan = scan_to_array(prev_scan)  # in local rectangular coords
        prior_scan = local_to_world(prev_scan, prev_pose)  # in world coords
        robot_scan = (data[n+2]['scan'])  # in local polar coords
        curr_scan = scan_to_array(robot_scan)  # in local rectangular coords
        robot_pose = data[n+2]['pose']
        synced_pose = sync_pose(robot_scan, robot_pose)  # estimated pose at time of mid-scan
        diff_pose = ph.calculate_relative_pose(prev_pose, synced_pose)
        prev_pose = synced_pose
        print(f"{diff_pose = }")
        icp_pose = slam_front_end.point_to_plane_icp(curr_scan.T, prev_scan.T, diff_pose)
        print(f"{icp_pose = }")
        print()
